...
"""
function factor(f :: P, prime :: Int) :: Vector{Tuple{P, Int}} where P <: Union{Polynomial, PolynomialSparse}
...
function expand_factorization(
        ...
        ) :: P where P <: Union{Polynomial, PolynomialSparse}
...
"""
function multiplicity(f :: P, g :: P, prime :: Int) :: Int where P <: Union{Polynomial, PolynomialSparse}
...
"""
function dd_factor(f :: P, prime :: Int) :: Array{P} where P <: Union{Polynomial, PolynomialSparse}
    x = x_poly()
    w = deepcopy(x)
...
"""
function dd_split(f :: P, d :: Int, prime :: Int) :: Vector{P} where P <: Union{Polynomial, PolynomialSparse}
    f = mod(f,prime)
    degree(f) == d && return [f]
    degree(f) == 0 && return []
